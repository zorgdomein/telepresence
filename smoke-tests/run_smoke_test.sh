...
prepare_license_config() {
    ...
    yq e '.cloud.skipLogin = true' -i ~/Library/ApplicationSupport/telepresence/config.yml
    yq e '.cloud.systemaHost = "127.0.0.1"' -i ~/Library/ApplicationSupport/telepresence/config.yml
    yq e '.cloud.systemaPort = 456' -i ~/Library/ApplicationSupport/telepresence/config.yml
    yq e '.images.agentImage = "ambassador-telepresence-agent:1.11.2"' -i ~/Library/ApplicationSupport/telepresence/config.yml
    yq e '.images.registry = "datawire"' -i ~/Library/ApplicationSupport/telepresence/config.yml
...
if [[ -z $TELEPRESENCE_LICENSE ]]; then
    echo "You must provide a TELEPRESENCE_LICENSE to run the smoke tests"
    exit 1
fi
